...
describe('scripts/build-all.sh - --api-out is honored without --api-entry', () => {
  ...
  function runWithStubs(extraArgs: string[]) {
    ...
    writeFileSync(
      join(scripts, 'build-all.sh'),
      readFileSync(join(ROOT, 'scripts/build-all.sh'), 'utf8'),
    );
...
describe('scripts/offline-smoke.sh - rejects remote-CDN output', () => {
  test('fails when index.html references a remote CDN asset', () => {
    ...
    try {
      ...
      writeFileSync(
        join(site, 'search/search_index.json'),
        JSON.stringify({ docs: [{ location: '', title: 'x', text: 'y' }] }),
      );
      writeFileSync(
  ...
  test('passes on a self-contained site with a non-empty search index', () => {
    ...
    try {
      ...
      writeFileSync(
        join(site, 'search/search_index.json'),
        JSON.stringify({ docs: [{ location: '', title: 'Home', text: 'CuraOS' }] }),
      );
      writeFileSync(
        join(site, 'index.html'),
        `<head><link rel="stylesheet" href="assets/main.css"></head><body>CuraOS</body>`,
      );
      const r = sh('scripts/offline-smoke.sh', ['--site', site]);
